...
function CompileStats {
    ...
    echo -e "ReferenceType\tSample\tReferenceID\tDepth\tBreadth\tSimilarity\tMappedReads\tProportionMapped\tDedupedReads\tMappedDupRate\tStringDupRate\tEstOverallDupRate" >| "$outFile"
    ...
    for refLabel in "$@"; do
        ...
        while IFS=":" read -r sID fq1 fq2; do
            ...
            local pS="-";
            targetFile="../output_data/TRACES_preprocessed_reads/$sID.duprate.txt"
            if [ -f "$targetFile" ]; then
                pS=$(awk '($2 != "NA"){printf("%0.02f%%",($2+0)*100);next} {printf("-")}' "$targetFile");
            fi
            local pDA="-"
            if [ "$pD" != "-" ] || [ "$pS" != "-" ]; then
                pDA=$(awk -v pd="$pD" -v ps="$pS" 'function rmp(x){return (0+substr(x,1,length(x)-1))/100} BEGIN{ps=rmp(ps); pd=rmp(pd); printf("%0.02f%%",100*((1-ps)*pd+ps))}')
            fi
            ...
            if [ "$gid" != "-" ]; then
                echo -e "$c1\t$sID\t$gid\t$depth\t$breadth\t$sim\t$nM\t$pM\t$nD\t$pD\t$pS\t$pDA";
            fi
        done < "$MetaFile"
    done >> "$outFile";
}
